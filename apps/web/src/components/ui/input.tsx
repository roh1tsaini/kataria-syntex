...
function Input({ className, type, ...props }: React.ComponentProps<"input">) {
  return (
    <input
      ...
      data-slot="input"
      className={cn(
        "h-11 w-full rounded-control border border-line bg-paper px-3.5 text-sm text-navy shadow-xs transition-[border-color,box-shadow] duration-200 placeholder:text-ink-soft/60",
