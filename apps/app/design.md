...
| Settings window (`settings-window.tsx`)              | Company settings renders as a large window at `md`+, not a centered dialog: pinned below the chrome strip (`top: calc(3.5rem + 12px)`), `max-w-[56rem]`, `max-h-[calc(100dvh-5rem)]` with its own internal scroller, radius 16px, `--shadow-overlay`. Same scrim as Dialog (40% + 4px blur) covers the app beneath; the window's own close is a `CircleButton` top-right. Enter/exit on `MORPH`/`MORPH_EXIT`, reduced-motion collapses to opacity. At `≤sm` the same component renders the page form — one surface, breakpoint-decided, no platform branch (Section 4.1).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
