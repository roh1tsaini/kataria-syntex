...
import { cacheMasters, readMasters } from "@/lib/offline/core";
...
export const useMasters = create<MastersState>()((set, get) => {
  ...
  // Boot from the offline cache so pickers work without a server; each
  // refresh overwrites both the store and the cache when online.
