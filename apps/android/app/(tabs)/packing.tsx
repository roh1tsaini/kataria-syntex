 ...
 * numbers via the same API calls) and copy. Also exports PackingImportDialog
 * — the "Import from Packing" sheet the challan editor mounts.
 */
...
// ── Import from Packing (mounted by the challan editor) ─────────────────────
...
/**
 * Import packed sale items as challan rows — the Modal-sheet port of
 * apps/app's packing-import-dialog (same flow and copy). Owns its list
...
export function PackingImportDialog({
  ...
  const [entries, setEntries] = useState<PackingImportEntry[]>([]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      setLoadError(false);
      try {
        const res = await api<{ items: PackingImportEntry[] }>(
          "/packing?type=sale",
        );
